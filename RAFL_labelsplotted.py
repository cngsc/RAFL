...
def segmentation(img):
    ...
    fImg = binary(bImg, T)
    fImg = fill_hole(fImg)
    dist_visual, labels, mask, markers = watershed(fImg, segmentation_footprint,segmentation_minimum_distance)
    return fImg, dist_visual, labels, mask, markers
...
def watershed(fImg, segmentation_footprint,segmentation_minimum_distance):
    ...
    markers, no_of_labels = scipy.ndimage.label(mask, structure=np.ones((3, 3)))
    ...
    nuclei_count=(no_of_labels)

    return dist_visual, labels, mask, markers
...
def plot_overlay(background, masks, titles, filename, channel, site_markers):
    fig = plt.figure()
    fig.set_size_inches(16, 12)
    nimages = len(masks)

    for i in range(len(masks)): # should be 2 since there is a cyto mask and a nuclear mask
        masked = np.ma.masked_where(masks[i] == 0, masks[i]) #basically makes all non mask values transparent when you plot it later
        ...
        #now plot the number labels ontop of the overlayed figure 
        for row in range(masked.shape[0]):
            for col in range(masked.shape[1]):
                if site_markers[row, col] != 0: #avoid bg
                    plt.text(col, row, str(site_markers[row, col]), color='white', ha='center', va='center', path_effects=[withStroke(linewidth=3, foreground='black')])
...
def initialize_all_labels(path, cell_growth_size_pixels):
    ...
    for file in os.listdir(path):
        ...
        fImg, dist_visual, labels, mask, markers = segmentation(img)
        ...
        adi_dict[site] = adi
        da_dict[site] = da
        #write the markers array for each dapi img into an overarching dict where key = site eg "1-1" - used to label overlay images
        markers_dict[site]=markers
    ...
    return dapi_dict, l_adi, adi_dict, nlabels_dict, clabels_dict, da_dict, markers_dict

def gate_by_bounds(leftboundx, rightboundx, nlabels, clabels, d, da, markers):
    ...
    for y in range(rows):
        for x in range(cols):        
            ...
            if clabels[y][x] in delabels: #do the same for the cell labels 
                gated_clabels[y][x] = 0
            if markers[y][x] in delabels: #do the same for the cell labels 
                gated_markers[y][x] = 0

    return gated_nlabels, gated_clabels, gated_markers
...
def get_all_labels(path, gatebyfixedvalues):
    ...
    dapi_dict, l_adi, adi_dict, nlabels_dict, clabels_dict, da_dict, markers_dict = initialize_all_labels(path, cell_growth_size_pixels)
    ...
    gated_markers_dict = {} # initiate a new dictionary to hold gated marker arrays
    for site in list(nlabels_dict.keys()):
        ...
        gated_nlabels, gated_cytolabels, gated_markers = gate_by_bounds(leftboundx, rightboundx, nlabels, cytolabs, adi, da, markers)
        ...
        gated_markers_dict[site] = gated_markers
    ...
    return gated_nlabels_dict, gated_cytolabels_dict, clabels_dict, gated_markers_dict
...
def summing_image_intensities(
    ...
    site_markers = gated_markers_dict[site]
    ...
    plot_overlay(img, masks, titles , site, channel, site_markers)

def main():

    start_time = time.time()

    gated_nlabels_dict, gated_cytolabels_dict, clabels_dict, gated_markers = get_all_labels(path, gatebyfixedvalues)
